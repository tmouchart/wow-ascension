// Felsworn Tyrant WeakAura v5: slimmer, gradient-filled Energy bar + Felfury boxes
// + Health bar (top->bottom: Energy / Felfury / Health) + cooldown icon row.
const fs = require('fs');
...
const FEL_HI = [0.45, 0.90, 0.06, 1];    // felfury / health bright green
const FEL_LO = [0.10, 0.32, 0.0, 1];     // felfury / health dark green
const HP_HI = [0.90, 0.16, 0.12, 1];     // health bright red
const HP_LO = [0.33, 0.02, 0.02, 1];     // health dark red

// --- bar geometry (slimmer than v4) ---
const BAR_W = 300;
...
for (let i = 1; i <= MAX_FELFURY; i++) {
  ...
  // empty by default; a condition paints a green gradient when the stack is present
...
// ---------- Health bar ----------
const health = baseBar('Felsworn Health', 'felswrnHp001');
...
const ICONS = [
  ...
  // Fel Fireball is a baseline ability (not in the talent scrape); id 1970 resolves no
  // texture on the CoA client -> track by name and give a fallback icon so it never blanks.
  { label: 'Fel Fireball', spell: 'Fel Fireball', byName: true, glowBuff: 'Carve',
...
group.id = GROUP_ID; group.uid = 'felswrnGrp005';
...
stripMeta(group);
group.controlledChildren = [energy.id, ...felBoxes.map(b => b.id), health.id, cdGroup.id];

// flat list of ALL regions
const children = [energy, ...felBoxes, health, cdGroup, ...iconRegions];
const top = { d: group, c: children, m: 'd', s: '5.20.2', v: 2000 };

fs.writeFileSync('felsworn-v5.decoded.json', JSON.stringify(top, null, 2));
const str = encodeWA(top);
fs.writeFileSync('felsworn-v5.import.txt', str);
const ok = JSON.stringify(decodeWA(str).data) === JSON.stringify(top);
console.log('v5 written: felsworn-v5.import.txt (' + str.length + ' chars) | self round-trip:', ok);
