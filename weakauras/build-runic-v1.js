// Runemaster / Runic WeakAura v1 — built on the Felsworn v6 pattern.
// Layout (top -> bottom), same arrangement as Felsworn:
//   [ primary cooldowns / proc / Runic Brand ]   <- top icon row
//   [ MANA bar (blue gradient) ]
//   [ HEALTH bar (red gradient) ]
//   [ secondary cooldowns ]                       <- bottom icon row
//
// Centerpiece = Runic Brand: an icon tracking the DEBUFF ON THE TARGET with its stack
// count (0->3) as a big number, glowing orange at 3 stacks (= "detonate with Runeblade").
// Runeblade also glows orange when the target's brand hits 3. Power Overwhelming is a
...
// --- geometry ---
const BAR_W = 300;
const MANA_H = 16, HEALTH_H = 14;
const ICON_SIZE = 32, ICON_SIZE_2 = 26;
const CD_Y = -145;        // top primary cooldown row
const MANA_Y = -182;      // mana bar
const HEALTH_Y = -202;    // health bar
const CD2_Y = -236;       // bottom secondary cooldown row
...
const ICONS_MAIN = [
  { label: 'Runic Brand', spell: 712299, brandStacks: true },                       // debuff tracker (stacks + glow@3)
