...
// validate (so we only hand back a SPEC the generator accepts). External / non-Auraforge strings that don't
...
serve({ fetch: app.fetch, port: PORT }, () => console.log(`auraforge agent (hono) listening on :${PORT}`));
